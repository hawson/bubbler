...
'''Find bubbles, based on the spectrogram.py script from sounddevice.'''
import argparse
import math
import shutil
...
parser.add_argument(
    '-b', '--block-duration', type=float, metavar='DURATION', default=100,
...
try:
    ...
    with sd.InputStream(device=args.device, channels=1, callback=callback,
                        ...
                        samplerate=samplerate, dither_off=True):
        while True:
            response = input()
            if response in ('', 'q', 'Q'):
                break
